...
module.exports = function (PersistObjectTemplate) {
    ...
    PersistObjectTemplate.getPOJOsFromKnexQuery = function (template, joins, queryOrChains, options, map, logger, projection) {
        ...
        joins.forEach(function (join) {
            select = select.leftOuterJoin(this.dealias(join.template.__table__) + ' as ' + join.alias, join.alias + '.' + join.parentKey, this.dealias(template.__table__) + '.' + join.childKey);
        }.bind(this));
        ...
        if (options && options.sort) {
            ...
            if (ascending.length)
                select = ascending.reduce(function (result, column) { return select.orderBy(column); }, select);
            if (descending.length)
                select = descending.reduce(function (result, column) { return select.orderBy(column, 'desc'); }, select);
        }
    ...
    PersistObjectTemplate.deleteFromKnexByQuery = function (template, queryOrChains, txn, _logger) {
        return __awaiter(this, void 0, void 0, function () {
            var deleteQueries, deleteQuery;
            return __generator(this, function (_a) {
                if (!txn) {
                    return [2 /*return*/, PersistObjectTemplate.deleteFromKnexQuery(template, queryOrChains, txn, _logger)];
                }
                deleteQueries = txn ? txn.deleteQueries : this.deleteQueries;
                deleteQuery = { name: template.__name__, template: template, queryOrChains: queryOrChains };
            ...
            function columnNameToProp(columnName) {
                ...
                for (var parent in schema.parents) {
                    if (columnName == schema.parents[parent].id && !props[parent]) {
                        PersistObjectTemplate.logger.info({ component: 'persistor', module: 'db.synchronizeKnexTableFromTemplate', activity: 'discoverColumns' }, 'schema out-of-sync: schema contains ' + columnName + ' on ' + table + ', which is not defined in the template');
                        return false;
                    }
    ...
    PersistObjectTemplate._commitKnex = function _commitKnex(persistorTransaction, logger, notifyChanges) {
        ...
        return knex.transaction(function (knexTransaction) {
            ...
            function rollback(err) {
                return __awaiter(this, void 0, void 0, function () {
                    var deadlock, remoteDocService_1, toDeletePromiseArr_1;
                    var _this = this;
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0:
                                ...
                                if (!(persistorTransaction.remoteObjects && persistorTransaction.remoteObjects.size > 0)) return [3 /*break*/, 2];
                                (logger || this.logger).info({
                                    component: 'persistor',
                                    module: 'api',
                                    activity: 'end'
                                }, "Rolling back transaction of remote keys");
                                remoteDocService_1 = RemoteDocService_1.RemoteDocService.new(this.environment, this.remoteDocHostURL);
                                toDeletePromiseArr_1 = [];
                                persistorTransaction.remoteObjects.forEach(function (versionId, key) {
                                    toDeletePromiseArr_1.push(remoteDocService_1.deleteDocument(key, _this.bucketName, versionId)
                                        .catch(function (error) {
                                        (logger || _this.logger).error({
                                            component: 'persistor',
                                            module: 'api',
                                            activity: 'end',
                                            error: error
                                        }, 'unable to rollback remote document with key:' + key + ' and bucket: ', _this.bucketName);
                                    }));
                                });
                                // fire off our delete requests in parallel
                                return [4 /*yield*/, Promise.all(toDeletePromiseArr_1)];
                            case 1:
                                // fire off our delete requests in parallel
                                _a.sent();
                                _a.label = 2;
                            case 2: return [2 /*return*/, knexTransaction.rollback(innerError).then(function () {
                                    (logger || _this.logger).debug({
                                        component: 'persistor',
                                        module: 'api',
                                        activity: 'end'
            ...
            function generateChanges(obj, action) {
                ...
                function generatePropertyChanges(prop, obj) {
                    ...
                    var replaceNullValuesWithUndefined = function (k, v) { return v === null ? undefined : v; };
